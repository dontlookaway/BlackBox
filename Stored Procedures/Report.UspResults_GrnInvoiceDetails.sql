...
///			14/01/2016	Chris Johnson			Added Approver	and Orig Receipt Date												///
///			15/01/2016	Chris Johnson			Added data to be stord locally and then displayed to speed up recovery of data		///
...
--list the tables that are to be pulled back from each DB - if they are not found the script will not be run against that db
		...
		Declare @LoadDate DateTime2 = GetDate(),@IsComplete Bit

--Create Results Table
		If Not Exists (Select [T].[name] From [sys].[schemas] As [S] Left Join sys.[tables] As [T] On [T].[schema_id] = [S].[schema_id]
					Where S.[name]='Report'
...
Insert [Process].[GrnInvoiceDetails]
        ( [LoadDate], [IsComplete], [Company])
Values  ( @LoadDate, 0, @Company)
...
--execute script against each db, populating the base tables
        Print 'A';
        Exec [sys].[sp_MSforeachdb] @SQL1a;
        Print 'B';
        Exec [sys].[sp_MSforeachdb] @SQL1b;
        Print 'C';
        Exec [sys].[sp_MSforeachdb] @SQL2;
        Print 'D';
        Exec [sys].[sp_MSforeachdb] @SQLReq;
        Print 'Run';
